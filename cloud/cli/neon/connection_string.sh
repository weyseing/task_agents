#!/usr/bin/env bash
# Print the DATABASE_URL connection string for a Neon project.
set -euo pipefail
...
usage() {
  cat <<EOF
Print the Postgres connection string for a Neon project — suitable for copying
into .env as DATABASE_URL or into Cloud Run env vars.
